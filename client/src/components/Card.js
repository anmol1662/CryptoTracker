  ...
  return (
    ...
    <div className="flex items-center gap-4">
      ...
      <div className="flex flex-col w-full">
        ...
        <button>{(result) ? (<MdOutlineStar onClick={watchlistRemove} className="text-[40px]" />) :(<FaRegStar onClick={watchlistAdd} className="text-[40px]" />)}</button>
      </div>

      <div className="flex items-center gap-4">
        { !checker && <h2 className="border rounded-3xl p-2 border-green-600 text-[10px] lg:text-[14px] text-green-800 ">
      ...
      <h2 className="text-[34px] font-bold  text-green-600 ">
        &#36;{checker === 'top15' ? Math.round(item.data.price * 1000000)/1000000 : Math.round(item.current_price * 1000000)/1000000}
      </h2>
      <div className="flex flex-col gap-1">
        <h2 className="text-[14px]">Market Capacity: {checker === 'top15' ? "":"$"}
          {checker === 'top15' ? item.data.market_cap : item.market_cap}</h2>
        <h2 className="text-[14px]">Total volume: {checker === 'top15' ? "" : "$"}
          {checker === 'top15' ? item.data.total_volume : item.total_volume}</h2>
      </div>
    </div>
  );
};
